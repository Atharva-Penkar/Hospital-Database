...
// --- MULTI-BACKEND URL LOGIC FOR TESTS ---
...
// You can change this logic to resolve by test name or other criteria
const resolveTestBaseByName = async (testName: string): Promise<string | null> => {
  for (const baseUrl of TEST_SERVICE_URLS) {
    try {
      const response = await fetch(`${baseUrl}/api/dbtest-available`);
      if (!response.ok) continue;
      const tests = await response.json();
      if (Array.isArray(tests)) {
        // Optionally, check if the test name exists at this URL
        // if (tests.some((t: any) => t.test_name === testName)) return baseUrl;
        return baseUrl;
      }
    } catch {
      continue;
...
const TestsTable: React.FC<TestsTableProps> = ({ darkMode }) => {
  ...
  // Fetch tests from all backend URLs in parallel
  const fetchTests = async (): Promise<void> => {
    setLoading(true);
    setError(null);
    try {
      const fetchPromises = TEST_SERVICE_URLS.map(async (baseUrl) => {
        try {
          const response = await fetch(`${baseUrl}/api/dbtest-available`);
      ...
      const transformedData: Test[] = results.flat().map((item: ApiTest) => ({
        id: item.T_ID,
        name: item.test_name
      }));
      setTests(transformedData);
  ...
  const addTest = async (test: Omit<Test, 'id'>): Promise<Test | undefined> => {
    try {
      setLoading(true);
      if (!test.name) throw new Error("Test name is required");
      const baseUrl = await resolveTestBaseByName(test.name);
      if (!baseUrl) throw new Error("No service found for the test name");
      const response = await fetch(`${baseUrl}/api/dbtest-available`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ test_name: test.name })
  ...
  const updateTest = async (id: number, testData: Partial<Test>): Promise<Test | undefined> => {
    try {
      setLoading(true);
      if (!testData.name) throw new Error("Test name is required for update");
      const baseUrl = await resolveTestBaseByName(testData.name);
      if (!baseUrl) throw new Error("No service found for the test name");
      const response = await fetch(`${baseUrl}/api/dbtest-available/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ test_name: testData.name })
      });
  ...
  const deleteTest = async (id: number): Promise<void> => {
    try {
      ...
      if (!testToDelete) throw new Error("Test not found");
      const baseUrl = await resolveTestBaseByName(testToDelete.name);
      if (!baseUrl) throw new Error("No service found for the test name");
      const response = await fetch(`${baseUrl}/api/dbtest-available/${id}`, {
        method: 'DELETE'
      });
